...
uint256 hashGenesisBlock("0x435662611cba108b1edb1a40a5bfb2307724bdbebe7d32b7a8a83405b53ef23c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // EmotiCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "EmotiCoin Signed Message:\n";
...
{
    // EmotiCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 500000 blocks, which will occur approximately every 1 years
    nSubsidy >>= (nHeight / 500000); // EmotiCoin: 500k blocks in ~1 years
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // EmotiCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // EmotiCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // EmotiCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Aliquam a posuere dui. Ut dignissim turpis eget sem bibendum, vitae malesuada sem volutpat.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1393632000;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 791196647;
        ...
        assert(block.hashMerkleRoot == uint256("0x248b6e4f52d8b7d63873e0a165edf2fd3759cae95a23d7be424e9f4c0aaf42da"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // EmotiCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EmotiCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EmotiCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EmotiCoinMiner(CWallet *pwallet)
{
    printf("EmotiCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("emoticoin-miner");
    ...
    try { loop {
        ...
        printf("Running EmotiCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EmotiCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EmotiCoinMiner, pwallet));
}
